...
Next, we can transform our dtaframe into a shapefile, to a visualize traject. We use `sf` package. throught `st_as_sf()` and `st_cast()` functions to turn our dataframe into a shapefile.
